...
class Board
  ...
  def valid_placement?(ship, coordinate_array)
    ...
    ship.length.times do |location|
      return false if letter[location].ord < 'A'.ord || letter[location].ord > 'D'.ord # This validates that the letter is between A-D as anything outside is not a valid coordinate
      return false if number[location] < 1 || number[location] > 4 # check if the number is less than 1 or greater than 4 since it's not a valid coordinate
